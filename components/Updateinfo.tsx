...
import { createPatient,  RegiserUser, UpdateUser } from "@/lib/actions/patient.action";
...
import { Patient, UpdatePatient } from "@/types/appwrite";
const Updatefrom = ({ patient, imageUrl}: { patient: Partial<UpdatePatient>, imageUrl:any }) => {
  ...
  const form = useForm<z.infer<typeof PatientFormValidation>>({
    ...
    defaultValues: {
      ...
      email: patient.email ?? "",
      phone: patient.phone ?? "",
      birthDate: new Date(patient.birthDate ?? Date.now()),
      gender: patient.gender as Gender ?? ("Male" as Gender),
...
//  console.log(patient.identificationDocumentUrl)
  async function onSubmit(values: z.infer<typeof PatientFormValidation>) {
    ...
    try {
      ...
      const data = {
        ...
        documentId: patient.$id as string,
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <div className=" flex flex-col mb-7 space-y-2  ">
          <h3 className=" font-bold text-[26px] ">Welcome Again 👋</h3>
          <p className=" text-xs text-zinc-400 ">
            We or update your information...
          </p>
        </div>
